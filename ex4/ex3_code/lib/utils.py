...
def sampleGaussian(mu, logvar):
    ...
    sample = mu + torch.exp(logvar) * torch.randn(size=logvar.shape)
...
def computeELBO(net, predictions, targets, device, args):
    ...
    # - MSE does not depend on number of weight samples, training samples (we take the mean) and it is 1-D
    # - This acts more like lambda putting more or less emphasis on the MSE loss compared to the prior matching term
    nll_scale = 10.0
...
def computeKLD(mean_a_flat, logvar_a_flat, device,
    ...
    kl = 0.5 * torch.mean( (logvar_a_flat - 1 +  (1 / torch.exp(logvar_a_flat)) * (1 + mean_a_flat**2) ) )
